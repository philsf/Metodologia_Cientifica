...
\begin{frame}{Muito trabalho}
  \begin{enumerate}
    \scriptsize
  \item<1,5> A pesquisa médica na mídia
  \item<1,2> Proposta de estudo 1/2
  \item<1,2> Elaboração de uma pergunta PICO
  \item<1,5> Posição de tópico, posição de ênfase
  \item<1,2> Proposta de estudo 2/2
  \item<1,3> Protocolo de pesquisa
  \item<1,5> Citações in loco
  \item<1,3> Relatório: Introdução + Métodos + Referências
  \item<1,4> Relatório: Resultados
  \item<1,4> Relatório: discussão + padronização da formatação + sumários
  \item<1,4> Relatório: resumo
  \item<1,4> Apresentação + relatório final
  \end{enumerate}

  \begin{center}
    \visible<1>{\footnotesize Você {\bf \normalsize experimentou} as etapas do método científico}
    ...
    \visible<5>{\footnotesize Tarefas de fixação (suporte)}
  \end{center}
\end{frame}
