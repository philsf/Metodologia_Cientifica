...
\begin{frame}{Qual foi o retorno?}
  \begin{block}{Aprendizado prático}
    {\scriptsize \centering Vocês viram a dificuldade da...}
    \bigskip
    \bigskip
    \begin{itemize}
      \scriptsize
    \item<1> ... formulação de uma pergunta {\bf respondível}
      \bigskip
    \item<2> ... delineação de um {\bf método} para atender ao objetivo
      \bigskip
    \item<3> {... redação de uma introdução que {\bf contextualiza e justifica} a importância do tema}
      \bigskip
    \item<4> ... redação do método com detalhamento necessário para {\bf reprodutibilidade} {\tiny (sistemáticooooooooooo...)}
      \bigskip
    \item<5> ... descrição e interpretação dos {\bf resultados} obtidos
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{E a correção te enfezava}
  \begin{center}
    \includegraphics[width=1.2\textwidth]{Encerramento/phdcomics-correcao}
