...
\begin{frame}{Estrutura típica de um projeto}
  \begin{itemize}
  \item Título ({\bf o quê?})
  \item Resumo
  \item Introdução / Justificativa (ou Motivação) ({\bf por quê})
  \item Revisão bibliográfica\footnote{por vezes chamado
      de Fundamentação Teórica}
  \item Objetivos ({\bf para quê?})
  \item Desenvolvimento
    \begin{itemize}
    \item Metodologia ou Materiais e Métodos ({\bf como?})
    \item Resultados preliminares (se disponíveis)
    \end{itemize}
  \item Cronograma de atividades ({\bf quando?})
  \item Referências
  \end{itemize}
