...
\begin{frame}{Buscas por perguntas estruturadas no PUBMED}
  \begin{itemize}
    \footnotesize
...
\begin{frame}{\footnotesize Principais características do estudo}
  \begin{itemize}
    \footnotesize
  \item Objetivo: Comparação das duas interfaces PICO ({\tiny em relação ao padrão})
    \bigskip
  \item Desfecho primário: precisão ({\tiny proporção de citações {\em relevantes} em relação ao total})
    \bigskip
  \item Resultado: maior precisão em ambos os PICO ({\tiny que na interface padrão})
    \bigskip
  \item Conclusão:
    \begin{itemize}
      \scriptsize
    ...
    \end{itemize}
    \bigskip
  \item Obs: links para as interfaces nas refs do paper
  \end{itemize}
